...
- 운영 로그인은 **Google / Kakao / Naver 소셜 로그인만** 지원합니다(아이디·비밀번호 로그인 미제공).
- 로컬/스테이징에서 테스트가 필요할 때만 `DEV_AUTH=1` + `DEV_AUTH_SECRET=...` 로 dev-login 폴백을 켤 수 있습니다.
- 결제 테스트가 목적이라면 토스 테스트 키와 `BILLING_MODE` 만 점검하면 됩니다(별도 자격 증명 계정 시드는 더 이상 사용하지 않음).
