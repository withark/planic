...
export default function CuesheetPage() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const emptyDoc = createEmptyDoc({
      ...
      eventType: baseData.eventType || cuesheetType,
      requirements: [baseData.requirements, cuesheetType ? `큐시트 형태: ${cuesheetType}` : '', notes].filter(Boolean).join('\n'),
    })
  ...
  const handleExcelDownload = useCallback(async () => {
    if (!doc) return
    try {
      await exportToExcel(doc, null, 'cuesheet')
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-500">행사 정보와 진행 유형을 입력하면 AI가 큐시트를 만들어 드립니다.</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-green-200 bg-green-50 p-5">
            <h2 className="mb-3 text-sm font-semibold text-green-900">AI 생성</h2>
            <button
