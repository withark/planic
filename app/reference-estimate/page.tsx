...
export default function ReferenceEstimatePage() {
  ...
  const inputRefModeText = '견적서 템플릿 고정 모드: 업로드한 견적서의 항목명/구성/단가 스타일을 견적 생성 기준으로 사용합니다.'
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-card">
            ...
            <p className="text-sm text-gray-600 mt-1">
              ...
              {activeRef
                ? `업로드 템플릿 적용 중 — ${activeRef.filename}`
                : '아직 견적 생성에 반영 중인 템플릿 파일이 없습니다. 업로드 후 「견적 생성에 반영」을 눌러 주세요.'}
            </p>
            <p className="mt-2 text-xs text-slate-600 border-l-2 border-primary-200 pl-3 leading-relaxed">
              <span className="font-medium text-slate-800">업로드 ≠ 견적 반영</span>입니다. 아래 파일 목록에서{' '}
              <span className="font-medium text-slate-800">「견적 생성에 반영」</span>까지 눌러야 템플릿이 적용됩니다.
            </p>

            <ol className="mt-5 grid gap-4 sm:grid-cols-2">
              <li className="rounded-xl border border-slate-100 bg-slate-50/80 p-4">
                <div className="text-xs font-bold uppercase tracking-wide text-primary-600">1 · 적용 방식</div>
                <p className="text-sm text-gray-600 mt-2">{inputRefModeText}</p>
                <p className="mt-3 inline-flex rounded-lg border border-primary-200 bg-primary-50 px-3 py-1.5 text-xs font-semibold text-primary-700">
                  템플릿 고정 모드 (항상 활성)
                </p>
              </li>
              <li className="rounded-xl border border-slate-100 bg-slate-50/80 p-4">
          ...
          <section className="rounded-2xl border border-gray-100 bg-white shadow-card overflow-hidden">
            <div className="p-5 border-b border-gray-100 bg-slate-50/50">
              ...
              <div className="text-sm text-gray-600 mt-1">
                항목명·카테고리 구성·단가 스타일을 추출해 견적 생성 템플릿으로 사용합니다. 큐시트·시나리오는 다른 메뉴에서 올려 주세요.
              </div>
            </div>
              ...
              {refs.length === 0 ? (
                <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-gradient-to-b from-gray-50 to-white px-6 py-12 text-center">
                  ...
                  <p className="text-sm text-gray-600 mt-2 max-w-md mx-auto">
                    ...
                    <> 업로드 후 목록에서 「견적 생성에 반영」을 눌러야 템플릿이 적용됩니다.</>
                  </p>
