...
export default function ProgramProposalGeneratorPage() {
  ...
  const fetchInit = useCallback(async () => {
    ...
    apiFetch<CompanySettings>('/api/settings')
      ...
      .catch((e) => warnDevFetchFailure('GET /api/settings (program-proposal-generator)', e))
  ...
  }, [doc, requestBaseFromDoc, showToast, sourceMode, topic, headcount, venue, goal, notes, taskOrderSummary, startSession, stillCurrent, clearAbortIfCurrent, flushQueuedIntoNotes])
  ...
  const handleDownloadWord = useCallback(async () => {
    if (!doc) {
      showToast('먼저 프로그램 제안을 생성해 주세요.')
      return
    }
    setDownloadingWord(true)
    try {
      await exportProgramProposalDocxFromDoc(doc, { includeQuote: false, company: companySettings })
            ...
            {generating ? (
              ...
              <div className="flex min-h-0 h-full flex-col gap-3 overflow-hidden">
                ...
                <GenerationResultNextSteps
                  ...
                  onRegenerate={() => void handleGenerateProgram()}
              ...
              <section className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-card">
                ...
                <div className="min-h-0 flex-1">
                  ...
                  loadPreviousLabel="과거 프로그램 제안 불러오기"
                  />
                </div>
              </section>
              </div>
