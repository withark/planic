...
export function CreateDocumentsHubBody() {
  ...
  const quickStartDocs = useMemo(
    () => CORE_DOCUMENT_HREFS.map((href) => CREATE_DOCUMENT_HUB_ITEMS.find((d) => d.href === href)).filter(Boolean),
    [],
  )
  const expandedDocs = useMemo(
    () => CREATE_DOCUMENT_HUB_ITEMS.filter((doc) => !CORE_DOCUMENT_HREFS.includes(doc.href as (typeof CORE_DOCUMENT_HREFS)[number])),
    [],
  )
    ...
    <>
      ...
      <section className="rounded-2xl border-2 border-primary-100 bg-white p-4 sm:p-5 shadow-card ring-1 ring-primary-50/70">
        ...
        <p className="mt-1 text-sm text-slate-600">
          핵심 {CORE_DOCUMENT_COUNT}종을 먼저 시작하고, 필요할 때 전체 {TOTAL_DOCUMENT_COUNT}종으로 확장하세요.
        </p>
      ...
      {expandedDocs.length > 0 ? (
        <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-card">
          <h3 className="text-sm font-bold text-slate-900">확장 문서 {expandedDocs.length}종</h3>
          <p className="mt-1 text-xs text-slate-600">기획 고도화 단계에서 이어서 사용하는 문서입니다.</p>
